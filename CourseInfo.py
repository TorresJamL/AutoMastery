...
class Course():
    ...
    def get_all_outcomes(self):
        ''' return dict pairing outcome IDs with names '''
        outcome_alignments_url = f"{self.PAGE_URL}/courses/{self.COURSE_ID}/outcome_alignments?student_id=89837"
        response = requests.get(outcome_alignments_url, headers=self.headers)

        outcome_ids = set(map(lambda k : k['learning_outcome_id'], response.json()))

        outcome_pair = {}
        for id in outcome_ids:
            outcomes_url = f"{self.PAGE_URL}/outcomes/{id}"
            outcomes = requests.get(outcomes_url, headers=self.headers).json()
            outcome_pair[id] = outcomes['title']
